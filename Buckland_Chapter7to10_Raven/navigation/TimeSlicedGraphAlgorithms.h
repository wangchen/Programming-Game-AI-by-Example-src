...
public:
  Graph_SearchAStar_TS(const graph_type & G, int source, int target)
...
std::list<PathEdge> Graph_SearchAStar_TS<graph_type, heuristic>::GetPathAsPathEdges() const
{
  ...
  while ((nd != m_iSource) && (m_ShortestPathTree[nd] != 0)) {
    path.push_front(PathEdge(
      m_Graph.GetNode(m_ShortestPathTree[nd]->From()).Pos(),
      m_Graph.GetNode(m_ShortestPathTree[nd]->To()).Pos(), m_ShortestPathTree[nd]->Flags(),
...
public:
  Graph_SearchDijkstras_TS(const graph_type & G, int source, int target)
  : Graph_SearchTimeSliced<Edge>(Dijkstra),
