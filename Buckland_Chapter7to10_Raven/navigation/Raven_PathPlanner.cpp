...
: m_pOwner(owner), m_NavGraph(m_pOwner->GetWorld()->GetMap()->GetNavGraph()), m_pCurrentSearch(NULL)
{
}

//-------------------------- dtor ---------------------------------------------
//-----------------------------------------------------------------------------
Raven_PathPlanner::~Raven_PathPlanner() { GetReadyForNewSearch(); }
...
double Raven_PathPlanner::GetCostToNode(unsigned int NodeIdx) const
{
  ...
  double cost = Vec2DDistance(m_pOwner->Pos(), m_NavGraph.GetNode(nd).Pos());
...
double Raven_PathPlanner::GetCostToClosestItem(unsigned int GiverType) const
{
  ...
  //type GiverType
  const Raven_Map::TriggerSystem::TriggerList & triggers =
    m_pOwner->GetWorld()->GetMap()->GetTriggers();

  Raven_Map::TriggerSystem::TriggerList::const_iterator it;
  for (it = triggers.begin(); it != triggers.end(); ++it) {
    if (((*it)->EntityType() == GiverType) && (*it)->isActive()) {
      double cost = m_pOwner->GetWorld()->GetMap()->CalculateCostToTravelBetweenNodes(
        nd, (*it)->GraphNodeIndex());
...
Raven_PathPlanner::Path Raven_PathPlanner::GetPath()
{
  ...
  path.push_front(PathEdge(m_pOwner->Pos(), GetNodePosition(closest), NavGraphEdge::normal));
  ...
  if (m_pCurrentSearch->GetType() == Graph_SearchTimeSliced<EdgeType>::AStar) {
    path.push_back(PathEdge(path.back().Destination(), m_vDestinationPos, NavGraphEdge::normal));
  }
...
int Raven_PathPlanner::CycleOnce() const
{
  ...
  if (result == target_not_found) {
    Dispatcher->DispatchMsg(
      SEND_MSG_IMMEDIATELY, SENDER_ID_IRRELEVANT, m_pOwner->ID(), Msg_NoPathAvailable,
      NO_ADDITIONAL_INFO);
  ...
  //let the bot know a path has been found
    ...
    Dispatcher->DispatchMsg(
      SEND_MSG_IMMEDIATELY, SENDER_ID_IRRELEVANT, m_pOwner->ID(), Msg_PathReady, pTrigger);
  }
...
//-----------------------------------------------------------------------------
Vector2D Raven_PathPlanner::GetNodePosition(int idx) const { return m_NavGraph.GetNode(idx).Pos(); }
