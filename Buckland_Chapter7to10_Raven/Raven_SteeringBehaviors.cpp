...
//------------------------------------------------------------------------
double Raven_Steering::ForwardComponent() { return m_pRaven_Bot->Heading().Dot(m_vSteeringForce); }

//--------------------------- SideComponent ------------------------------
//  returns the side component of the steering force
//------------------------------------------------------------------------
double Raven_Steering::SideComponent() { return m_pRaven_Bot->Side().Dot(m_vSteeringForce); }
...
//  force to add.
//------------------------------------------------------------------------
bool Raven_Steering::AccumulateForce(Vector2D & RunningTot, Vector2D ForceToAdd)
{
...
Vector2D Raven_Steering::Seek(const Vector2D & target)
{
  Vector2D DesiredVelocity =
    Vec2DNormalize(target - m_pRaven_Bot->Pos()) * m_pRaven_Bot->MaxSpeed();
...
//------------------------------------------------------------------------
Vector2D Raven_Steering::Arrive(const Vector2D & target, const Deceleration deceleration)
{
...
Vector2D Raven_Steering::Wander()
{
  ...
  Vector2D Target =
    PointToWorldSpace(target, m_pRaven_Bot->Heading(), m_pRaven_Bot->Side(), m_pRaven_Bot->Pos());
  ...
  for (unsigned int flr = 0; flr < m_Feelers.size(); ++flr) {
    ...
    for (unsigned int w = 0; w < walls.size(); ++w) {
      if (LineIntersection2D(
            m_pRaven_Bot->Pos(), m_Feelers[flr], walls[w]->From(), walls[w]->To(), DistToThisIP,
            point)) {
...
void Raven_Steering::CreateFeelers()
{
  ...
  m_Feelers[0] = m_pRaven_Bot->Pos() +
                 m_dWallDetectionFeelerLength * m_pRaven_Bot->Heading() * m_pRaven_Bot->Speed();
