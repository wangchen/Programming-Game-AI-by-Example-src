...
//----------------------------- AddWall ---------------------------------------
//-----------------------------------------------------------------------------
void Raven_Map::AddWall(std::ifstream & in) { m_Walls.push_back(new Wall2D(in)); }
...
//  from nd1 to nd2
//-----------------------------------------------------------------------------
double Raven_Map::CalculateCostToTravelBetweenNodes(int nd1, int nd2) const
{
  assert(
    nd1 >= 0 && nd1 < m_pNavGraph->NumNodes() && nd2 >= 0 && nd2 < m_pNavGraph->NumNodes() &&
...
void Raven_Map::PartitionNavGraph()
{
  ...
  m_pSpacePartition = new CellSpacePartition<NavGraph::NodeType *>(
    m_iSizeX, m_iSizeY, script->GetInt("NumCellsX"), script->GetInt("NumCellsY"),
...
//  all the triggers
//-----------------------------------------------------------------------------
void Raven_Map::UpdateTriggerSystem(std::list<Raven_Bot *> & bots) { m_TriggerSystem.Update(bots); }
