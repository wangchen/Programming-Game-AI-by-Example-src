...
void Raven_Game::NotifyAllBotsOfRemoval(Raven_Bot * pRemovedBot) const
{
  ...
  for (curBot; curBot != m_Bots.end(); ++curBot) {
    Dispatcher->DispatchMsg(
      SEND_MSG_IMMEDIATELY, SENDER_ID_IRRELEVANT, (*curBot)->ID(), Msg_UserHasRemovedBot,
      pRemovedBot);
  }
}
//-------------------------------RemoveBot ------------------------------------
...
//-----------------------------------------------------------------------------
void Raven_Game::RemoveBot() { m_bRemoveABot = true; }
...
bool Raven_Game::LoadMap(const std::string & filename)
{
  ...
  m_pPathManager =
    new PathManager<Raven_PathPlanner>(script->GetInt("MaxSearchCyclesPerUpdateStep"));
...
//  each point.
//-----------------------------------------------------------------------------
bool Raven_Game::isPathObstructed(Vector2D A, Vector2D B, double BoundingRadius) const
{
...
//-----------------------------------------------------------------------------
std::vector<Raven_Bot *> Raven_Game::GetAllBotsInFOV(const Raven_Bot * pBot) const
{
  ...
  for (curBot; curBot != m_Bots.end(); ++curBot) {
    ...
    if (isSecondInFOVOfFirst(pBot->Pos(), pBot->Facing(), (*curBot)->Pos(), pBot->FieldOfView())) {
      //cast a ray from between the bots to test visibility. If the bot is
      //visible add it to the vector
      if (!doWallsObstructLineSegment(pBot->Pos(), (*curBot)->Pos(), m_pMap->GetWalls())) {
...
bool Raven_Game::isSecondVisibleToFirst(const Raven_Bot * pFirst, const Raven_Bot * pSecond) const
{
  ...
  if (!(pFirst == pSecond) && pSecond->isAlive()) {
    //first of all test to see if this bot is within the FOV
    if (isSecondInFOVOfFirst(
          pFirst->Pos(), pFirst->Facing(), pSecond->Pos(), pFirst->FieldOfView())) {
      //test the line segment connecting the bot's positions against the walls.
      //If the bot is visible add it to the vector
      if (!doWallsObstructLineSegment(pFirst->Pos(), pSecond->Pos(), m_pMap->GetWalls())) {
        return true;
      }
    }
  }
...
//-----------------------------------------------------------------------------
Vector2D Raven_Game::GetPosOfClosestSwitch(Vector2D botPos, unsigned int doorID) const
{
...
void Raven_Game::Render()
{
  ...
  if (m_pSelectedBot && UserOptions->m_bOnlyShowBotsInTargetsFOV) {
    std::vector<Raven_Bot *> VisibleBots = GetAllBotsInFOV(m_pSelectedBot);
