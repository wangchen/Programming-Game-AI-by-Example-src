...
//------------------------------- ctor ----------------------------------------
//-----------------------------------------------------------------------------
Raven_SensoryMemory::Raven_SensoryMemory(Raven_Bot * owner, double MemorySpan)
: m_pOwner(owner), m_dMemorySpan(MemorySpan)
    ...
    if (m_pOwner != *curBot) {
      ...
      if (m_pOwner->GetWorld()->isLOSOkay(m_pOwner->Pos(), (*curBot)->Pos())) {
        ...
        if (isSecondInFOVOfFirst(
              m_pOwner->Pos(), m_pOwner->Facing(), (*curBot)->Pos(), m_pOwner->FieldOfView())) {
...
//-----------------------------------------------------------------------------
std::list<Raven_Bot *> Raven_SensoryMemory::GetListOfRecentlySensedOpponents() const
{
...
Vector2D Raven_SensoryMemory::GetLastRecordedPositionOfOpponent(Raven_Bot * pOpponent) const
{
  ...
  throw std::runtime_error(
    "< Raven_SensoryMemory::GetLastRecordedPositionOfOpponent>: Attempting to get position of "
    "unrecorded bot");
}
