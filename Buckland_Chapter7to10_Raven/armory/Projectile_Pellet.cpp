...
:
  ...
  Raven_Projectile(
    ...
    script->GetInt("Pellet_Damage"), script->GetDouble("Pellet_Scale"),
    script->GetDouble("Pellet_MaxSpeed"), script->GetDouble("Pellet_Mass"),
...
void Pellet::TestForImpact()
{
  ...
  FindClosestPointOfIntersectionWithWalls(
    m_vOrigin, m_vPosition, DistToClosestImpact, m_vImpactPoint, m_pWorld->GetMap()->GetWalls());
  ...
  GetLineSegmentCircleClosestIntersectionPoint(
    m_vOrigin, m_vImpactPoint, hit->Pos(), hit->BRadius(), m_vImpactPoint);
