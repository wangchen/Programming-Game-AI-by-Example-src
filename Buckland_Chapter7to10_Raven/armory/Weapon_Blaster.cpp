...
:
  ...
  Raven_Weapon(
    type_blaster, script->GetInt("Blaster_DefaultRounds"),
    script->GetInt("Blaster_MaxRoundsCarried"), script->GetDouble("Blaster_FiringFreq"),
    script->GetDouble("Blaster_IdealRange"), script->GetDouble("Bolt_MaxSpeed"), owner)
{
...
inline void Blaster::ShootAt(Vector2D pos)
{
  if (isReadyForNextShot()) {
    ...
    m_pOwner->GetWorld()->GetMap()->AddSoundTrigger(
      m_pOwner, script->GetDouble("Blaster_SoundRange"));
  }
}
