...
Raven_WeaponSystem::Raven_WeaponSystem(
  Raven_Bot * owner, double ReactionTime, double AimAccuracy, double AimPersistance)
: m_pOwner(owner),
...
void Raven_WeaponSystem::SelectWeapon()
{
  ...
  if (m_pOwner->GetTargetSys()->isTargetPresent()) {
    ...
    double DistToTarget =
      Vec2DDistance(m_pOwner->Pos(), m_pOwner->GetTargetSys()->GetTarget()->Pos());
...
void Raven_WeaponSystem::TakeAimAndShoot() const
{
  ...
  //or other cover)
  if (
    m_pOwner->GetTargetSys()->isTargetShootable() ||
    ...
    //target
    if (
      GetCurrentWeapon()->GetType() == type_rocket_launcher ||
...
Vector2D Raven_WeaponSystem::PredictFuturePositionOfTarget() const
{
  ...
  //agent's velocities
  double LookAheadTime = ToEnemy.Length() / (MaxSpeed + m_pOwner->GetTargetBot()->MaxSpeed());

  //return the predicted future position of the enemy
  return m_pOwner->GetTargetBot()->Pos() + m_pOwner->GetTargetBot()->Velocity() * LookAheadTime;
}
...
//-----------------------------------------------------------------------------
void Raven_WeaponSystem::ShootAt(Vector2D pos) const { GetCurrentWeapon()->ShootAt(pos); }

//-------------------------- RenderCurrentWeapon ------------------------------
//-----------------------------------------------------------------------------
void Raven_WeaponSystem::RenderCurrentWeapon() const { GetCurrentWeapon()->Render(); }
