  ...
  for (int o = 0; o < Prm.NumObstacles; ++o) {
    ...
    while (bOverlapped) {
      ...
      Obstacle * ob = new Obstacle(
        ...
        RandInt(radius + border, m_cyClient - radius - 30 - border), radius);
...
void GameWorld::HandleKeyPresses(WPARAM wParam)
{
  switch (wParam) {
    case 'U': {
      ...
      double border = 60;
      m_pPath =
        new Path(RandInt(3, 7), border, border, cxClient() - border, cyClient() - border, true);
      m_bShowPath = true;
...
void GameWorld::Render()
{
  ...
  for (unsigned int a = 0; a < m_Vehicles.size(); ++a) {
    ...
    if (m_bShowCellSpaceInfo && a == 0) {
      ...
      for (BaseGameEntity * pV = CellSpace()->begin(); !CellSpace()->end();
           pV = CellSpace()->next()) {
