...
//------------------------------------------------------------------------
double SteeringBehavior::ForwardComponent() { return m_pVehicle->Heading().Dot(m_vSteeringForce); }

//--------------------------- SideComponent ------------------------------
//  returns the side component of the steering force
//------------------------------------------------------------------------
double SteeringBehavior::SideComponent() { return m_pVehicle->Side().Dot(m_vSteeringForce); }
...
Vector2D SteeringBehavior::CalculateWeightedSum()
{
  ...
  if (On(obstacle_avoidance)) {
    m_vSteeringForce +=
      ObstacleAvoidance(m_pVehicle->World()->Obstacles()) * m_dWeightObstacleAvoidance;
  }
...
Vector2D SteeringBehavior::CalculateDithered()
{
  ...
  if (On(wall_avoidance) && RandFloat() < Prm.prWallAvoidance) {
    m_vSteeringForce =
      WallAvoidance(m_pVehicle->World()->Walls()) * m_dWeightWallAvoidance / Prm.prWallAvoidance;
  ...
  if (!isSpacePartitioningOn()) {
    if (On(separation) && RandFloat() < Prm.prSeparation) {
      m_vSteeringForce +=
        Separation(m_pVehicle->World()->Agents()) * m_dWeightSeparation / Prm.prSeparation;
    ...
    if (On(separation) && RandFloat() < Prm.prSeparation) {
      m_vSteeringForce +=
        SeparationPlus(m_pVehicle->World()->Agents()) * m_dWeightSeparation / Prm.prSeparation;
  ...
  if (!isSpacePartitioningOn()) {
    if (On(allignment) && RandFloat() < Prm.prAlignment) {
      m_vSteeringForce +=
        Alignment(m_pVehicle->World()->Agents()) * m_dWeightAlignment / Prm.prAlignment;
    ...
    if (On(cohesion) && RandFloat() < Prm.prCohesion) {
      m_vSteeringForce +=
        Cohesion(m_pVehicle->World()->Agents()) * m_dWeightCohesion / Prm.prCohesion;
    ...
    if (On(allignment) && RandFloat() < Prm.prAlignment) {
      m_vSteeringForce +=
        AlignmentPlus(m_pVehicle->World()->Agents()) * m_dWeightAlignment / Prm.prAlignment;
    ...
    if (On(cohesion) && RandFloat() < Prm.prCohesion) {
      m_vSteeringForce +=
        CohesionPlus(m_pVehicle->World()->Agents()) * m_dWeightCohesion / Prm.prCohesion;
  ...
  if (On(arrive) && RandFloat() < Prm.prArrive) {
    m_vSteeringForce +=
      Arrive(m_pVehicle->World()->Crosshair(), m_Deceleration) * m_dWeightArrive / Prm.prArrive;
...
Vector2D SteeringBehavior::Wander()
{
  ...
  m_vWanderTarget +=
    Vector2D(RandomClamped() * JitterThisTimeSlice, RandomClamped() * JitterThisTimeSlice);
  ...
  Vector2D Target =
    PointToWorldSpace(target, m_pVehicle->Heading(), m_pVehicle->Side(), m_pVehicle->Pos());
...
Vector2D SteeringBehavior::ObstacleAvoidance(const std::vector<BaseGameEntity *> & obstacles)
{
  ...
  if (ClosestIntersectingObstacle) {
    ...
    SteeringForce.y =
      (ClosestIntersectingObstacle->BRadius() - LocalPosOfClosestObstacle.y) * multiplier;
    ...
    SteeringForce.x =
      (ClosestIntersectingObstacle->BRadius() - LocalPosOfClosestObstacle.x) * BrakingWeight;
  }
  ...
  for (unsigned int flr = 0; flr < m_Feelers.size(); ++flr) {
    ...
    for (unsigned int w = 0; w < walls.size(); ++w) {
      if (LineIntersection2D(
            m_pVehicle->Pos(), m_Feelers[flr], walls[w].From(), walls[w].To(), DistToThisIP,
            point)) {
...
Vector2D SteeringBehavior::Separation(const vector<Vehicle *> & neighbors)
{
  ...
  for (unsigned int a = 0; a < neighbors.size(); ++a) {
    ...
    if (
      (neighbors[a] != m_pVehicle) && neighbors[a]->IsTagged() &&
...
Vector2D SteeringBehavior::SeparationPlus(const vector<Vehicle *> & neighbors)
{
  ...
  for (BaseGameEntity * pV = m_pVehicle->World()->CellSpace()->begin();
       !m_pVehicle->World()->CellSpace()->end(); pV = m_pVehicle->World()->CellSpace()->next()) {
    //make sure this agent isn't included in the calculations and that
...
//------------------------------------------------------------------------
Vector2D SteeringBehavior::GetHidingPosition(
  const Vector2D & posOb, const double radiusOb, const Vector2D & posHunter)
{
...
Vector2D SteeringBehavior::OffsetPursuit(const Vehicle * leader, const Vector2D offset)
{
  ...
  Vector2D WorldOffsetPos =
    PointToWorldSpace(offset, leader->Heading(), leader->Side(), leader->Pos());
...
void SteeringBehavior::RenderAids()
{
  ...
  if (On(wander) && m_pVehicle->World()->RenderWanderCircle()) {
    ...
    Vector2D m_vTCC = PointToWorldSpace(
      Vector2D(m_dWanderDistance * m_pVehicle->BRadius(), 0), m_pVehicle->Heading(),
      m_pVehicle->Side(), m_pVehicle->Pos());
    ...
    gdi->Circle(
      PointToWorldSpace(
        (m_vWanderTarget + Vector2D(m_dWanderDistance, 0)) * m_pVehicle->BRadius(),
  ...
  if (m_pVehicle->World()->RenderDetectionBox()) {
    ...
    if (!m_pVehicle->isSmoothingOn()) {
      ...
      box = WorldTransform(
        box, m_pVehicle->Pos(), m_pVehicle->SmoothedHeading(),
        m_pVehicle->SmoothedHeading().Perp());
      gdi->ClosedShape(box);
    }
