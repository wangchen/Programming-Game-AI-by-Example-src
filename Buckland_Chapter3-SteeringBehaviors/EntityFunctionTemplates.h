...
bool Overlapped(const T * ob, const conT & conOb, double MinDistBetweenObstacles = 40.0)
{
  ...
  for (it = conOb.begin(); it != conOb.end(); ++it) {
    if (TwoCirclesOverlapped(
          ob->Pos(), ob->BRadius() + MinDistBetweenObstacles, (*it)->Pos(), (*it)->BRadius())) {
      return true;
    }
  }
...
//-----------------------------------------------------------------------------
template <class T, class conT>
std::list<T> GetEntityLineSegmentIntersections(
  const conT & entities, int the_one_to_ignore, Vector2D A, Vector2D B, double range = MaxDouble)
{
  typename conT::const_iterator it;
  ...
  for (it = entities.begin(); it != entities.end(); ++it) {
    ...
    if (((*it)->ID() == the_one_to_ignore) || (Vec2DDistanceSq((*it)->Pos(), A) > range * range)) {
      continue;
    }
...
template <class T, class conT>
T * GetClosestEntityLineSegmentIntersection(
  const conT & entities, int the_one_to_ignore, Vector2D A, Vector2D B, double range = MaxDouble)
{
  typename conT::const_iterator it;
