...
void ResizeToCorrectClientArea(HWND hwnd, int AccumulativeToolbarHeight, RECT ClientArea)
{
  ...
  SetWindowPos(
    hwnd, NULL, 0, 0, ClientArea.right, ClientArea.bottom + AccumulativeToolbarHeight,
...
HWND CreateToolBar(HWND hwndParent, HINSTANCE hinstMain)
{
  ...
  HWND hwndToolBar = CreateWindowEx(
    NULL, TOOLBARCLASSNAME, (LPSTR)NULL, WS_CHILD | WS_VISIBLE | CCS_BOTTOM, 0, 0, 0, 0, hwndParent,
    (HMENU)IDR_TOOLBAR1, hinstMain, NULL);
  ...
  TBADDBITMAP tb;
  tb.hInst = NULL;
  tb.nID = (UINT_PTR)LoadBitmap(
    (HINSTANCE)GetWindowLong(hwndParent, GWL_HINSTANCE), MAKEINTRESOURCE(IDR_TOOLBAR1));
