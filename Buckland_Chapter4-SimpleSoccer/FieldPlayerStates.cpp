...
bool GlobalPlayerState::OnMessage(FieldPlayer * player, const Telegram & telegram)
{
  switch (telegram.Msg) {
    ...
    case Msg_PassToMe: {
      ...
      debug_con << "Player " << player->ID() << " received request from " << receiver->ID()
                << " to make pass"
                << "";
#endif
      ...
      Dispatcher->DispatchMsg(
        SEND_MSG_IMMEDIATELY, player->ID(), receiver->ID(), Msg_ReceiveBall, &receiver->Pos());
...
void ReturnToHomeRegion::Execute(FieldPlayer * player)
{
  if (player->Pitch()->GameOn()) {
    ...
    if (
      player->isClosestTeamMemberToBall() && (player->Team()->Receiver() == NULL) &&
...
void Wait::Execute(FieldPlayer * player)
{
  ...
  if (
    player->Team()->InControl() && (!player->isControllingPlayer()) &&
...
void KickBall::Execute(FieldPlayer * player)
{
  ...
  //to make the shot
  if (
    player->Team()->CanShoot(player->Ball()->Pos(), power, BallTarget) ||
  ...
  if (
    player->isThreatened() &&
    player->Team()->FindPass(player, receiver, BallTarget, power, Prm.MinPassDist)) {
...
#ifdef PLAYER_STATE_INFO_ON
    debug_con << "Player " << player->ID() << " passes the ball with force " << power
              << "  to player " << receiver->ID() << "  Target is " << BallTarget << "";
#endif

    //let the receiver know a pass is coming
    Dispatcher->DispatchMsg(
      SEND_MSG_IMMEDIATELY, player->ID(), receiver->ID(), Msg_ReceiveBall, &BallTarget);
...
void ReceiveBall::Enter(FieldPlayer * player)
{
  ...
  if (
    (player->InHotRegion() || RandFloat() < Prm.ChanceOfUsingArriveTypeReceiveBehavior) &&
