...
void ReturnHome::Enter(GoalKeeper * keeper) { keeper->Steering()->ArriveOn(); }
...
void ReturnHome::Exit(GoalKeeper * keeper) { keeper->Steering()->ArriveOff(); }
...
void InterceptBall::Exit(GoalKeeper * keeper) { keeper->Steering()->PursuitOff(); }
...
void PutBallBackInPlay::Execute(GoalKeeper * keeper)
{
  ...
  if (keeper->Team()->FindPass(
        keeper, receiver, BallTarget, Prm.MaxPassingForce, Prm.GoalkeeperMinPassDist)) {
    //make the pass
    ...
    Dispatcher->DispatchMsg(
      SEND_MSG_IMMEDIATELY, keeper->ID(), receiver->ID(), Msg_ReceiveBall, &BallTarget);
