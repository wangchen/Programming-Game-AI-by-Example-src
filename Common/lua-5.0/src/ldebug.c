...
LUA_API const char * lua_getlocal(lua_State * L, const lua_Debug * ar, int n)
{
  ...
  if (fp) { /* is a Lua function? */
    ...
    if (name) luaA_pushobject(L, ci->base + (n - 1)); /* push value */
  }
  lua_unlock(L);
  return name;
}
...
static const char * travglobals(lua_State * L, const TObject * o)
{
  ...
  while (i--) {
    Node * n = gnode(g, i);
    if (luaO_rawequalObj(o, gval(n)) && ttisstring(gkey(n))) return getstr(tsvalue(gkey(n)));
  }
  return NULL;
}
...
LUA_API int lua_getinfo(lua_State * L, const char * what, lua_Debug * ar)
{
  ...
  if (*what == '>') {
    StkId f = L->top - 1;
    if (!ttisfunction(f)) luaG_runerror(L, "value for `lua_getinfo' is not a function");
    ...
    switch (getOpMode(op)) {
      case iABC: {
        ...
        if (testOpMode(op, OpModeBreg)) {
          ...
          check(checkRK(pt, b));
        if (testOpMode(op, OpModeCrk)) check(checkRK(pt, c));
        break;
      }
      case iABx: {
        b = GETARG_Bx(i);
    ...
    *name = luaF_getlocalname(p, stackpos + 1, pc);
    ...
    switch (GET_OPCODE(i)) {
      ...
      case OP_MOVE: {
        ...
        if (b < a) return getobjname(ci, b, name); /* get name for `b' */
        break;
      }
      case OP_GETTABLE: {
