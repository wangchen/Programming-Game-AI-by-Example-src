...
static int listlocals(lua_State * L)
{
  ...
  luaL_argcheck(L, lua_isfunction(L, 1) && !lua_iscfunction(L, 1), 1, "Lua function expected");
  p = clvalue(func_at(L, 1))->l.p;
  while ((name = luaF_getlocalname(p, ++i, pc)) != NULL) lua_pushstring(L, name);
  return i - 1;
}
...
static int table_query(lua_State * L)
{
  ...
  if (i == -1) {
    ...
    if (
      !ttisnil(gval(gnode(t, i))) || ttisnil(gkey(gnode(t, i))) || ttisnumber(gkey(gnode(t, i)))) {
...
static int metatable(lua_State * L)
{
  luaL_checkany(L, 1);
  if (lua_isnone(L, 2)) {
    if (lua_getmetatable(L, 1) == 0) lua_pushnil(L);
  } else {
    lua_settop(L, 2);
    luaL_checktype(L, 2, LUA_TTABLE);
...
static int doonnewstack(lua_State * L)
{
  ...
  if (status == 0) status = lua_pcall(L1, 0, 0, 0);
...
static int doremote(lua_State * L)
{
  ...
  if (status == 0) status = lua_pcall(L1, 0, LUA_MULTRET, 0);
  if (status != 0) {
    lua_pushnil(L);
    lua_pushintegral(L, status);
    ...
    int i = 0;
    while (!lua_isnone(L1, ++i)) lua_pushstring(L, lua_tostring(L1, i));
    lua_pop(L1, i - 1);
    return i - 1;
  }
}
...
static const char * getname_aux(char * buff, const char ** pc)
{
  int i = 0;
  skip(pc);
  while (**pc != '\0' && !strchr(delimits, **pc)) buff[i++] = *(*pc)++;
  buff[i] = '\0';
  return buff;
}
...
static int testC(lua_State * L)
{
  ...
  for (;;) {
    ...
    if EQ ("")
      ...
      if (lua_getmetatable(L, getnum) == 0) lua_pushnil(L);
    } else if EQ ("type") {
...
{
  if (!islocked) lua_close(lua_state);
...
int main(int argc, char * argv[])
{
  ...
  if (limit) memdebug_memlimit = strtoul(limit, NULL, 10);
  l_main(argc, argv);
  return 0;
}
