...
void luaK_nil(FuncState * fs, int from, int n)
{
  ...
  if (
    fs->pc > fs->lasttarget && /* no jumps to current position? */
...
static void luaK_patchlistaux(
  FuncState * fs, int list, int ttarget, int treg, int ftarget, int freg, int dtarget)
{
  while (list != NO_JUMP) {
...
void luaK_checkstack(FuncState * fs, int n)
{
  ...
  if (newstack > fs->f->maxstacksize) {
    if (newstack >= MAXSTACK) luaX_syntaxerror(fs->ls, "function or expression too complex");
...
{
  if (e->k == VNONRELOC) freereg(fs, e->info);
}
...
static void discharge2reg(FuncState * fs, expdesc * e, int reg)
{
  ...
  switch (e->k) {
    ...
    case VNONRELOC: {
      if (reg != e->info) luaK_codeABC(fs, OP_MOVE, reg, e->info, 0);
      break;
    }
    default: {
...
static void luaK_exp2reg(FuncState * fs, expdesc * e, int reg)
{
  ...
  if (e->k == VJMP) luaK_concat(fs, &e->t, e->info); /* put this jump in `t' list */
  if (hasjumps(e)) {
