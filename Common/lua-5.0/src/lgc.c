...
#define condmarkobject(st, o, c)                                                            \
  {                                                                                         \
    checkconsistency(o);                                                                    \
    if (iscollectable(o) && !ismarked(gcvalue(o)) && (c)) reallymarkobject(st, gcvalue(o)); \
  }
...
static void removekey(Node * n)
{
  ...
  if (iscollectable(gkey(n))) setttype(gkey(n), LUA_TNONE); /* dead key; remove it */
}
...
static void traversetable(GCState * st, Table * h)
{
  ...
  if (!weakvalue) {
    i = h->sizearray;
    while (i--) markobject(st, &h->array[i]);
  }
  i = sizenode(h);
  while (i--) {
    Node * n = gnode(h, i);
...
static void traverseproto(GCState * st, Proto * f)
{
  ...
  for (i = 0; i < f->sizek; i++) { /* mark literal strings */
    if (ttisstring(f->k + i)) stringmark(tsvalue(f->k + i));
  }
...
static void traversestack(GCState * st, lua_State * L1)
{
  ...
  for (ci = L1->base_ci; ci <= L1->ci; ci++) {
    ...
    if (!(ci->state & CI_C) && lim < ci->top) lim = ci->top;
  }
  for (o = L1->stack; o < L1->top; o++) markobject(st, o);
...
{
  if (ttisstring(o)) stringmark(tsvalue(o)); /* strings are `values', so are never weak */
