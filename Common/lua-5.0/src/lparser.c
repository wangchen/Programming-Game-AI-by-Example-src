...
#define enterlevel(ls) \
  if (++(ls)->nestlevel > LUA_MAXPARSERLEVEL) luaX_syntaxerror(ls, "too many syntax levels");
...
{
  if (!testnext(ls, c)) error_expected(ls, c);
}
...
static void check_match(LexState * ls, int what, int who, int where)
{
  if (!testnext(ls, what)) {
    if (where == ls->linenumber)
      ...
      luaX_syntaxerror(
        ls, luaO_pushfstring(
              ls->L, "`%s' expected (to close `%s' at line %d)", luaX_token2str(ls, what),
...
static void singlevaraux(FuncState * fs, TString * n, expdesc * var, int base)
{
  if (fs == NULL)                   /* no more levels? */
    ...
    if (v >= 0) {
      ...
      if (var->k == VGLOBAL) {
        if (base) var->info = luaK_stringK(fs, n); /* info points to global name */
      } else {                                     /* LOCAL or UPVAL */
...
static void body(LexState * ls, expdesc * e, int needself, int line)
{
  ...
  check(ls, '(');
  if (needself) create_local(ls, "self");
  parlist(ls);
  check(ls, ')');
  chunk(ls);
...
static void prefixexp(LexState * ls, expdesc * v)
{
  ...
  switch (ls->t.token) {
    ...
    case '%': { /* for compatibility only */
      ...
      if (v->k != VUPVAL) luaX_errorline(ls, "global upvalues are obsolete", getstr(varname), line);
      return;
    }
#endif
    default: {
...
static void expr(LexState * ls, expdesc * v) { subexpr(ls, v, -1); }
...
static void whilestat(LexState * ls, int line)
{
  ...
  if (sizeexp > MAXEXPWHILE) luaX_syntaxerror(ls, "`while' condition too complex");
  ...
  for (i = 0; i < sizeexp; i++) luaK_code(fs, codeexp[i], lineexp);
...
static int funcname(LexState * ls, expdesc * v)
{
  ...
  int needself = 0;
  singlevar(ls, v, 1);
  while (ls->t.token == '.') luaY_field(ls, v);
  if (ls->t.token == ':') {
    needself = 1;
    luaY_field(ls, v);
  }
