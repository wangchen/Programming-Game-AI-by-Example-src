...
static int aux_close(lua_State * L)
{
  ...
  if (f == stdin || f == stdout || f == stderr)
    ...
    if (ok) *(FILE **)lua_touserdata(L, 1) = NULL; /* mark file as closed */
    return ok;
  }
}
...
static int f_flush(lua_State * L) { return pushresult(L, fflush(tofile(L, 1)) == 0, NULL); }
...
static const luaL_reg flib[] = {
  {"flush", f_flush}, {"read", f_read},    {"lines", f_lines}, {"seek", f_seek},
  {"write", f_write}, {"close", io_close}, {"__gc", io_gc},    {"__tostring", io_tostring},
  {NULL, NULL}};
...
static int getfield(lua_State * L, const char * key, int d)
{
  ...
  if (lua_isnumber(L, -1))
    ...
    if (d == -2) return luaL_error(L, "field `%s' missing in date table", key);
    res = d;
  }
  lua_pop(L, 1);
  return res;
