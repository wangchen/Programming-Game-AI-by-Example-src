...
static int luaB_setmetatable(lua_State * L)
{
  ...
  if (luaL_getmetafield(L, 1, "__metatable")) luaL_error(L, "cannot change a protected metatable");
  lua_settop(L, 2);
...
static void getfunc(lua_State * L)
{
  if (lua_isfunction(L, 1))
    ...
    lua_getinfo(L, "f", &ar);
    if (lua_isnil(L, -1)) luaL_error(L, "no function environment for tail call at level %d", level);
  }
}
...
{
  luaL_checkany(L, 1);
  if (!lua_toboolean(L, 1)) return luaL_error(L, "%s", luaL_optstring(L, 2, "assertion failed!"));
  lua_settop(L, 1);
  return 1;
}
...
static int luaB_require(lua_State * L)
{
  ...
  switch (status) {
    ...
    case LUA_ERRFILE: { /* file not found */
      return luaL_error(
        L, "could not load package `%s' from path `%s'", lua_tostring(L, 1), getpath(L));
    }
    default: {
      return luaL_error(
        L, "error loading package `%s' (%s)", lua_tostring(L, 1), lua_tostring(L, -1));
    }
  }
}
...
static int auxresume(lua_State * L, lua_State * co, int narg)
{
  ...
  if (status == 0) {
    ...
    if (!lua_checkstack(L, nres)) luaL_error(L, "too many results to resume");
...
static const luaL_reg co_funcs[] = {{"create", luaB_cocreate}, {"wrap", luaB_cowrap},
