...
static int str_lower(lua_State * L)
{
  ...
  const char * s = luaL_checklstring(L, 1, &l);
  luaL_buffinit(L, &b);
  for (i = 0; i < l; i++) luaL_putchar(&b, tolower(uchar(s[i])));
  luaL_pushresult(&b);
  return 1;
}
...
static int str_upper(lua_State * L)
{
  ...
  const char * s = luaL_checklstring(L, 1, &l);
  luaL_buffinit(L, &b);
  for (i = 0; i < l; i++) luaL_putchar(&b, toupper(uchar(s[i])));
  luaL_pushresult(&b);
  return 1;
}
...
static int str_dump(lua_State * L)
{
  ...
  luaL_buffinit(L, &b);
  if (!lua_dump(L, writer, &b)) luaL_error(L, "unable to dump given function");
  luaL_pushresult(&b);
  return 1;
}
...
static const char * luaI_classend(MatchState * ms, const char * p)
{
  switch (*p++) {
    case ESC: {
      if (*p == '\0') luaL_error(ms->L, "malformed pattern (ends with `%')");
      return p + 1;
    }
    case '[': {
      if (*p == '^') p++;
      do { /* look for a `]' */
        if (*p == '\0') luaL_error(ms->L, "malformed pattern (missing `]')");
...
static const char * max_expand(MatchState * ms, const char * s, const char * p, const char * ep)
{
  ...
  while ((s + i) < ms->src_end && luaI_singlematch(uchar(*(s + i)), p, ep)) i++;
...
static int push_captures(MatchState * ms, const char * s, const char * e)
{
  ...
  if (ms->level == 0 && s) {          /* no explicit captures? */
    ...
    for (i = 0; i < ms->level; i++) push_onecapture(ms, i);
...
static int gfind_aux(lua_State * L)
{
  ...
  ms.src_init = s;
  ms.src_end = s + ls;
  for (src = s + (size_t)lua_tonumber(L, lua_upvalueindex(3)); src <= ms.src_end; src++) {
    const char * e;
    ms.level = 0;
...
static const luaL_reg strlib[] = {
  {"len", str_len},   {"sub", str_sub},   {"lower", str_lower}, {"upper", str_upper},
  {"char", str_char}, {"rep", str_rep},   {"byte", str_byte},   {"format", str_format},
  {"dump", str_dump}, {"find", str_find}, {"gfind", gfind},     {"gsub", str_gsub},
  {NULL, NULL}};
