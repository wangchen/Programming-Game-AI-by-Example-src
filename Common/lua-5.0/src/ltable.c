...
static int arrayindex(const TObject * key)
{
  if (ttisnumber(key)) {
    ...
    if (cast(lua_Number, k) == nvalue(key) && k >= 1 && !toobig(k)) return k;
  }
...
static int luaH_index(lua_State * L, Table * t, StkId key)
{
  ...
  if (0 <= i && i <= t->sizearray) { /* is `key' inside array part? */
    ...
    if (v == &luaO_nilobject) luaG_runerror(L, "invalid key for `next'");
    i = cast(
      int, (cast(const lu_byte *, v) - cast(const lu_byte *, gval(gnode(t, 0)))) / sizeof(Node));
...
static void resize(lua_State * L, Table * t, int nasize, int nhsize)
{
  ...
  if (nasize < oldasize) { /* array part must shrink? */
    ...
    for (i = nasize; i < oldasize; i++) {
      if (!ttisnil(&t->array[i])) setobjt2t(luaH_setnum(L, t, i + 1), &t->array[i]);
    }
    /* shrink array */
  ...
  for (i = twoto(oldhsize) - 1; i >= 0; i--) {
    Node * old = nold + i;
    if (!ttisnil(gval(old))) setobjt2t(luaH_set(L, t, gkey(old)), gval(old));
  }
  if (oldhsize) luaM_freearray(L, nold, twoto(oldhsize), Node); /* free old array */
}
...
{
  if (t->lsizenode) luaM_freearray(L, t->node, sizenode(t), Node);
