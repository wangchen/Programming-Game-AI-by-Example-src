...
static void callTM(
  lua_State * L, const TObject * f, const TObject * p1, const TObject * p2, const TObject * p3)
{
  setobj2s(L->top, f);      /* push function */
  setobj2s(L->top + 1, p1); /* 1st argument */
  setobj2s(L->top + 2, p2); /* 2nd argument */
...
static const TObject * luaV_index(lua_State * L, const TObject * t, TObject * key, int loop)
{
...
static const TObject * luaV_getnotable(lua_State * L, const TObject * t, TObject * key, int loop)
{
  const TObject * tm = luaT_gettmbyobj(L, t, TM_INDEX);
  if (ttisnil(tm)) luaG_typeerror(L, t, "index");
  if (ttisfunction(tm)) {
    callTMres(L, tm, t, key);
    return L->top;
  } else
...
void luaV_settable(lua_State * L, const TObject * t, TObject * key, StkId val)
{
  ...
  do {
    if (ttistable(t)) { /* `t' is a table? */
      ...
      if (
        !ttisnil(oldval) ||                                    /* result is no nil? */
...
retentry: /* entry point when returning to old functions */
  ...
  for (;;) {
    ...
    lua_assert(
      L->top == L->ci->top || GET_OPCODE(i) == OP_CALL || GET_OPCODE(i) == OP_TAILCALL ||
    ...
    switch (GET_OPCODE(i)) {
      ...
      case OP_UNM: {
        ...
        if (tonumber(rb, &temp)) {
          ...
          setnilvalue(&temp);
          if (!call_binTM(L, RB(i), &temp, ra, TM_UNM)) luaG_aritherror(L, RB(i), &temp);
        }
        break;
      }
      case OP_NOT: {
      ...
      case OP_RETURN: {
        ...
        if (!(ci->state & CI_CALLING)) {
          ...
          int nresults;
          lua_assert(ci->u.l.pc == &pc && ttisfunction(ci->base - 1) && (ci->state & CI_SAVEDPC));
      ...
      case OP_FORLOOP: {
        ...
        if (!ttisnumber(ra)) luaG_runerror(L, "`for' initial value must be a number");
      ...
      case OP_SETLISTO: {
        ...
        for (; n > 0; n--) setobj2t(luaH_setnum(L, h, bc + n), ra + n); /* write barrier */
        break;
      }
      case OP_CLOSE: {
        luaF_close(L, ra);
