...
static void luaD_growCI(lua_State * L)
{
  if (L->size_ci > LUA_MAXCALLS) /* overflow while handling overflow? */
    ...
    if (L->size_ci > LUA_MAXCALLS) luaG_runerror(L, "stack overflow");
  }
}
...
StkId luaD_precall(lua_State * L, StkId func)
{
  ...
  if (!cl->isC) { /* Lua function? prepare its call */
    ...
    ci->u.l.tailcalls = 0;
    ci->state = CI_SAVEDPC;
    while (L->top < ci->top) setnilvalue(L->top++);
    L->top = ci->top;
    return NULL;
...
void luaD_poscall(lua_State * L, int wanted, StkId firstResult)
{
  ...
  while (wanted-- > 0) setnilvalue(res++);
  L->top = res;
}
...
static void resume(lua_State * L, void * ud)
{
  ...
  if (ci == L->base_ci) { /* no activation record? */
    if (nargs >= L->top - L->base) luaG_runerror(L, "cannot resume dead coroutine");
    ...
    if (ci->state & CI_C) {                /* `common' yield? */
      ...
      lua_assert(
        GET_OPCODE(*((ci - 1)->u.l.savedpc - 1)) == OP_CALL ||
...
LUA_API int lua_yield(lua_State * L, int nresults)
{
  ...
  lua_lock(L);
  ci = L->ci;
  if (L->nCcalls > 0) luaG_runerror(L, "attempt to yield across metamethod/C-call boundary");
  if (ci->state & CI_C) { /* usual yield */
    if ((ci - 1)->state & CI_C) luaG_runerror(L, "cannot yield a C function");
