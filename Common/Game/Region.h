...
public:
  ...
  Region(double left, double top, double right, double bottom, int id = -1)
  : m_dTop(top), m_dRight(right), m_dLeft(left), m_dBottom(bottom), m_iID(id)
  {
...
{
  return Vector2D(RandInRange(m_dLeft, m_dRight), RandInRange(m_dTop, m_dBottom));
}
...
inline bool Region::Inside(Vector2D pos, region_modifier r = normal) const
{
  if (r == normal) {
    ...
    return (
      (pos.x > (m_dLeft + marginX)) && (pos.x < (m_dRight - marginX)) &&
