...
void convert_cpp_to_lua(lua_State * L, const boost::intrusive_ptr<T> & ptr)
{
  ...
  new (obj) luabind::detail::object_rep(
    raw_ptr, crep, detail::object_rep::owner, decrement_ref_count<T>::apply);
...
template <class T>
boost::intrusive_ptr<T> convert_lua_to_cpp(
  lua_State * L, by_const_reference<boost::intrusive_ptr<T> >, int index)
{
  typename detail::default_policy::template generate_converter<T *, detail::lua_to_cpp>::type
    converter;
...
template <class T>
boost::intrusive_ptr<T> convert_lua_to_cpp(
  lua_State * L, by_value<boost::intrusive_ptr<T> >, int index)
{
...
void f(boost::intrusive_ptr<A> ptr) { std::cout << "count: " << ptr->cnt << "\n"; }

boost::intrusive_ptr<A> factory() { return boost::intrusive_ptr<A>(new A()); }
...
int main()
{
  ...
  class_<A>(L, "A").def_readonly("cnt", &A::cnt);
