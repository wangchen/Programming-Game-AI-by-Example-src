...
void displayfunc_callback() { displayfunc(); }
...
void idlefunc_callback() { idlefunc(); }
...
void reshapefunc_callback(int w, int h) { reshapefunc(w, h); }

void set_reshapefunc(const luabind::functor<void> & fun) { reshapefunc = fun; }

luabind::functor<void> keyboardfunc;

void keyboardfunc_callback(unsigned char key, int x, int y) { keyboardfunc(key, x, y); }
...
void mousefunc_callback(int button, int state, int x, int y) { mousefunc(button, state, x, y); }

void set_mousefunc(const luabind::functor<void> & fun) { mousefunc = fun; }
}  // namespace glut_bindings
...
void bind_glut(lua_State * L)
{
  ...
  class_<glut_constants>(L, "glut").enum_("constants")
    [value("RGB", GLUT_RGB), value("RGBA", GLUT_RGBA), value("INDEX", GLUT_INDEX),
     value("SINGLE", GLUT_SINGLE), value("DOUBLE", GLUT_DOUBLE), value("DEPTH", GLUT_DEPTH),
     value("STENCIL", GLUT_STENCIL), value("LEFT_BUTTON", GLUT_LEFT_BUTTON),
     value("MIDDLE_BUTTON", GLUT_MIDDLE_BUTTON), value("RIGHT_BUTTON", GLUT_RIGHT_BUTTON),
     value("UP", GLUT_UP), value("DOWN", GLUT_DOWN), value("ELAPSED_TIME", GLUT_ELAPSED_TIME)];
  ...
  class_<gl_constants>(L, "gl").enum_("constants")
    [value("COLOR_BUFFER_BIT", GL_COLOR_BUFFER_BIT), value("DEPTH_BUFFER_BIT", GL_DEPTH_BUFFER_BIT),
     value("TRIANGLES", GL_TRIANGLES), value("MODELVIEW", GL_MODELVIEW),
