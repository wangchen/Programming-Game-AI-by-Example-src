...
inline double DegsToRads(double degs) { return TwoPi * (degs / 360.0); }

//returns true if the parameter is equal to zero
inline bool IsZero(double val) { return ((-MinDouble < val) && (val < MinDouble)); }
...
{
  // 下記参照, typenameかclassの指定が必要 --> c++11以降はautoでも可
