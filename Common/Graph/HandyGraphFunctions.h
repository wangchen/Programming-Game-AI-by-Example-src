...
void GraphHelper_AddAllNeighboursToGridNode(
  graph_type & graph, int row, int col, int NumCellsX, int NumCellsY)
{
  for (int i = -1; i < 2; ++i) {
    for (int j = -1; j < 2; ++j) {
      int nodeX = col + j;
...
//-----------------------------------------------------------------------------
template <class graph_type>
void GraphHelper_CreateGrid(
  graph_type & graph, int cySize, int cxSize, int NumCellsY, int NumCellsX)
{
...
void WeightNavGraphNodeEdges(graph_type & graph, int node, double weight)
{
  ...
  for (const graph_type::EdgeType * pE = ConstEdgeItr.begin(); !ConstEdgeItr.end();
